...
async def send_voice_message(
    request: Request,
    voice: UploadFile = File(..., description="Audio file (mp3, wav, etc.) to send to the chatbot"),  # Required voice file
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    **Send Voice Message to Chatbot**
    ...
    - Accepts audio/voice file only
    - STT (Speech-to-Text): Converts voice to text
    - AI agent processes the text and generates response
    ...
    **Returns:** Current chat + previous chat history (last 20 chats) + audio response URL
    ...
    try:
        ...
        # Validate and process message
        message_text = gemini_service.validate_and_process_message(message_text.strip())
